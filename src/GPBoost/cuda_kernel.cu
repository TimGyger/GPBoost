...
namespace GPBoost {
    ...
    bool try_update_resid_diag_gpu(
        vec_t& D,             // length N, will be updated in-place
        const den_mat_t& A // size M×N
    ) {
        ...
        // Host pointers
        const double* h_sigma = A.data();
        double* h_resid = D.data();

        // Device buffers
        double* d_sigma = nullptr, * d_diag = nullptr;
        cublasHandle_t cublasH = nullptr;

        const size_t matBytes = size_t(M) * N * sizeof(double);
        const size_t vecBytes = size_t(N) * sizeof(double);

        // 1) Allocate GPU memory
        if (cudaMalloc(&d_sigma, matBytes) != cudaSuccess ||
            cudaMalloc(&d_diag, vecBytes) != cudaSuccess) {
            Log::REInfo("[GPU] cudaMalloc failed");
            return false;
        }

        // 2) Copy inputs to device
        if (cudaMemcpy(d_sigma, h_sigma, matBytes, cudaMemcpyHostToDevice) != cudaSuccess ||
            cudaMemcpy(d_diag, h_resid, vecBytes, cudaMemcpyHostToDevice) != cudaSuccess) {
            Log::REInfo("[GPU] cudaMemcpy H2D failed");
            return false;
        }

        // 3) Create cuBLAS handle
        if (cublasCreate(&cublasH) != CUBLAS_STATUS_SUCCESS) {
            Log::REInfo("[GPU] cublasCreate failed");
            return false;
        }

        // 4) Compute N self-dots (one per column) via batched DDOT
        //    cublasDdotStridedBatched(handle,
        //                              M,                // vector length
        //                              x, incx, strideX, // pointer/inc/stride for each vector
        //                              x, incy, strideY, // same pointer/inc/stride for y=x
        //                              batchCount,       // # of vectors (N)
        //                              result_ptr)       // output array of length N
        const int     inc = 1;
        const long long stride = M;  // columns are M entries apart
        const int     batch = N;
        if (cublasDdotStridedBatched(
            cublasH,
            M,
            d_sigma, inc, stride,
            d_sigma, inc, stride,
            batch,
            d_diag) != CUBLAS_STATUS_SUCCESS)
        {
            Log::REInfo("[GPU] cublasDdotStridedBatched failed");
            return false;
        }

        // 5) Copy the computed diag-sums back into host scratch (we overwrite h_resid,
        //    then subtract in Eigen)
        if (cudaMemcpy(h_resid, d_diag, vecBytes, cudaMemcpyDeviceToHost) != cudaSuccess) {
            Log::REInfo("[GPU] cudaMemcpy D2H failed");
            return false;
        }

        // 6) Subtract: resid[ii] -= sum_j sigma(j,ii)^2
        D.array() -= Eigen::Map<Eigen::VectorXd>(h_resid, N).array();

        // Success
        Log::REInfo("[GPU] Residual-diag update completed on GPU.");
