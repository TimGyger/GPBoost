		...
		while ((int)(cover_tree.size() - 1) != (int)(coords.rows())) {
			...
			for (const auto& key_covert_points_old_i : covert_points_old) {
				...
				while (not_all_covered) {
					...
					if (covert_points_old_i_up.size() > 0) {
...
#pragma omp parallel for
					for (int i = 0; i < num_threads; ++i) {
						...
						CoverTree_kNN(coords_ct.middleRows(segment_start[i], segment_length[i]), chol_ip_cross_cov.middleCols(segment_start[i], segment_length[i]),
							corr_diag.segment(segment_start[i], segment_length[i]), base, segment_start[i], re_comps_vecchia_cluster_i, cover_trees[i],
					...
					if (num_threads != 1) {
						...
						for (int ii = 0; ii < neighbors_per_tree.size(); ++ii) {
							if ((segment_start[ii] + num_neighbors) < i) {
								find_kNN_CoverTree(i, num_neighbors, levels_threads[ii], save_distances, base, coords, chol_ip_cross_cov,
						...
						find_kNN_CoverTree(i, num_neighbors, level, save_distances, base,
...
#pragma omp for schedule(static)
	...
	void UpdateNearestNeighbors(std::vector<std::shared_ptr<RECompGP<den_mat_t>>>& re_comps_vecchia_cluster_i,
		...
		std::vector<den_mat_t>& dist_between_neighbors_cluster_i,
		bool save_distances_isotropic_cov_fct) {
		std::shared_ptr<RECompGP<den_mat_t>> re_comp = std::dynamic_pointer_cast<RECompGP<den_mat_t>>(re_comps_vecchia_cluster_i[ind_intercept_gp]);
		...
		if (gp_approx == "full_scale_vecchia" && vecchia_neighbor_selection == "residual_correlation") {
			find_nearest_neighbors_Vecchia_FSA_fast(re_comp->GetCoords(), num_re, num_neighbors, chol_ip_cross_cov,
				re_comps_vecchia_cluster_i, nearest_neighbors_cluster_i, dist_obs_neighbors_cluster_i, dist_between_neighbors_cluster_i, 0, -1, has_duplicates, rng, save_distances_isotropic_cov_fct,
				2., false, false, num_re);
		}
		else {
		...
		if (CondObsOnly) {
			if (gp_approx == "full_scale_vecchia" && vecchia_neighbor_selection == "residual_correlation") {
				find_nearest_neighbors_Vecchia_FSA_fast(coords_all, num_re_cli + num_re_pred_cli, num_neighbors_pred, chol_ip_cross_cov_obs_pred,
					re_comps_vecchia[cluster_i], nearest_neighbors_cluster_i, dist_obs_neighbors_cluster_i, dist_between_neighbors_cluster_i, num_re_cli,
					num_re_cli - 1, check_has_duplicates, rng, distances_saved, 2., true, false, (int)num_re_cli);
			}
			else {
				if (!scale_coordinates) {
			...
			if (gp_approx == "full_scale_vecchia" && vecchia_neighbor_selection == "residual_correlation") {
				find_nearest_neighbors_Vecchia_FSA_fast(coords_all, num_re_cli + num_re_pred_cli, num_neighbors_pred, chol_ip_cross_cov_obs_pred,
					re_comps_vecchia[cluster_i], nearest_neighbors_cluster_i, dist_obs_neighbors_cluster_i, dist_between_neighbors_cluster_i, num_re_cli,
					-1, check_has_duplicates, rng, distances_saved, 2., true, true, (int)num_re_cli);
			}
			else {
				if (!scale_coordinates) {
					find_nearest_neighbors_Vecchia_fast(coords_all, num_re_cli + num_re_pred_cli, num_neighbors_pred,
