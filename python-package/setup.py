...
GPBOOST_OPTIONS = [
    ...
    ('cuda_gp', None, 'Compile CUDA version for GP'),
...
def compile_cpp(
    ...
    if system() in {'Windows', 'Microsoft'}:
        if use_mingw:
            ...
            ## Installion using VS solution currently not supported
            # if not any((use_gpu, use_cuda, use_cuda_gp, use_mpi, use_hdfs, nomp, bit32, integrated_opencl)):
...
class CustomInstall(install):
    ...
    def run(self) -> None:
        ...
        if not self.precompile:
            ...
            compile_cpp(use_mingw=self.mingw, use_gpu=self.gpu, use_cuda=self.cuda, use_cuda_gp=self.cuda_gp, use_mpi=self.mpi,
